...
fun abort_migration__should_fail_if_the_pending_version_is_not_this_package_version() {
    ...
    scenario.next_tx(OWNER);
    start_migration_to_custom_version_for_testing(
        &scenario,
        version_control::current_version() + 100,
    );
...
fun test_start_migration(scenario: &mut Scenario) {
    ...
    assert_eq(
        last_event_by_type(),
        treasury::create_migration_started_event<TREASURY_MIGRATION_TESTS>(
...
fun test_abort_migration(scenario: &mut Scenario) {
    ...
    assert_eq(
        last_event_by_type(),
        treasury::create_migration_aborted_event<TREASURY_MIGRATION_TESTS>(
            updated_compatible_versions,
        ),
    );
...
fun test_complete_migration(scenario: &mut Scenario) {
    ...
    assert_eq(
        last_event_by_type(),
        treasury::create_migration_completed_event<TREASURY_MIGRATION_TESTS>(
            updated_compatible_versions,
        ),
    );
