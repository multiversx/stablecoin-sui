...
use sui::test_utils::{Self, assert_eq, destroy};
use sui::vec_set;
...
fun configure_controller__should_fail_if_caller_is_not_master_minter() {
    ...
    {
        let mut treasury = scenario.take_shared<Treasury<TREASURY_TESTS>>();
        treasury.configure_controller(
            RANDOM_ADDRESS,
            object::id_from_address(MINT_CAP_ADDR),
            scenario.ctx(),
        );
        test_scenario::return_shared(treasury);
    };
...
fun configure_controller__should_fail_if_treasury_object_is_incompatible() {
    let (
        ...
        deny_list,
        metadata,
    ) = before_incompatible_treasury_object_scenario();
    treasury.configure_controller(
        RANDOM_ADDRESS,
        object::id_from_address(RANDOM_ADDRESS_2),
        scenario.ctx(),
    );
...
fun before_incompatible_treasury_object_scenario(): (
    Scenario,
    Treasury<TREASURY_TESTS>,
    DenyList,
    CoinMetadata<TREASURY_TESTS>,
) {
    let mut scenario = setup();
    ...
    scenario.next_tx(OWNER);
    let mut treasury = scenario.take_shared<Treasury<TREASURY_TESTS>>();
    treasury.set_compatible_versions_for_testing(
        vec_set::singleton(version_control::current_version() + 1),
    );
    test_scenario::return_shared(treasury);
...
fun after_incompatible_treasury_object_scenario(
    scenario: Scenario,
    treasury: Treasury<TREASURY_TESTS>,
    deny_list: DenyList,
    metadata: CoinMetadata<TREASURY_TESTS>,
) {
    test_scenario::return_shared(treasury);
...
fun test_configure_new_controller(controller: address, minter: address, scenario: &mut Scenario) {
    ...
    let expected_event2 = treasury::create_controller_configured_event<TREASURY_TESTS>(
        controller,
        mint_cap_id,
    );
...
fun test_configure_controller(controller: address, mint_cap_id: ID, scenario: &mut Scenario) {
    ...
    let expected_event = treasury::create_controller_configured_event<TREASURY_TESTS>(
        controller,
        mint_cap_id,
    );
...
fun test_configure_minter(allowance: u64, scenario: &mut Scenario) {
    ...
    let expected_event = treasury::create_minter_configured_event<TREASURY_TESTS>(
        scenario.sender(),
        mint_cap_id,
        allowance,
    );
...
fun test_increment_mint_allowance(
    ...
    let expected_event = treasury::create_minter_allowance_incremented_event<TREASURY_TESTS>(
        scenario.sender(),
        mint_cap_id,
        allowance_increment,
        expected_allowance,
...
fun test_remove_minter(scenario: &mut Scenario) {
    ...
    let expected_event = treasury::create_minter_removed_event<TREASURY_TESTS>(
        scenario.sender(),
        mint_cap_id,
    );
    assert_eq(event::num_events(), 1);
...
fun test_mint(mint_amount: u64, recipient: address, scenario: &mut Scenario) {
    ...
    let expected_event = treasury::create_mint_event<TREASURY_TESTS>(
        object::id(&mint_cap),
        recipient,
        mint_amount,
    );
...
fun test_burn(scenario: &mut Scenario) {
    ...
    let expected_event = treasury::create_burn_event<TREASURY_TESTS>(
        object::id(&mint_cap),
        burn_amount,
    );
    assert_eq(event::num_events(), 1);
...
fun test_blocklist(addr: address, scenario: &mut Scenario) {
    ...
    let blocklisted_before = coin::deny_list_v2_contains_current_epoch<TREASURY_TESTS>(
        &deny_list,
        addr,
        scenario.ctx(),
    );
    ...
    assert_eq(
        coin::deny_list_v2_contains_current_epoch<TREASURY_TESTS>(&deny_list, addr, scenario.ctx()),
        blocklisted_before,
    );

    let expected_event = treasury::create_blocklisted_event<TREASURY_TESTS>(addr);
    let expected_event_count =
        1 + event::events_by_type<deny_list::PerTypeConfigCreated>().length();
...
fun test_pause(scenario: &mut Scenario) {
    let mut deny_list = scenario.take_shared<DenyList>();
    let mut treasury = scenario.take_shared<Treasury<TREASURY_TESTS>>();
    let paused_before = coin::deny_list_v2_is_global_pause_enabled_current_epoch<TREASURY_TESTS>(
        &deny_list,
        scenario.ctx(),
    );
    ...
    treasury.pause(&mut deny_list, scenario.ctx());
    assert_eq(
        coin::deny_list_v2_is_global_pause_enabled_next_epoch<TREASURY_TESTS>(&deny_list),
        true,
    );
    assert_eq(
