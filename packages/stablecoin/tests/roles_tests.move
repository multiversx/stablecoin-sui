...
public(package) fun test_transfer_ownership<T>(
    new_owner: address,
    roles: &mut Roles<T>,
...
public(package) fun test_accept_ownership<T>(roles: &mut Roles<T>, scenario: &mut Scenario) {
    ...
    let expected_event = two_step_role::create_role_transferred_event<OwnerRole<T>>(
        old_owner,
        *pending_owner.borrow(),
    );
    assert_eq(event::num_events(), 1);
    assert_eq(last_event_by_type(), expected_event);
}

public(package) fun test_update_master_minter<T>(
    ...
    let expected_event = roles::create_master_minter_changed_event<T>(
        old_master_minter,
        new_master_minter,
    );
    assert_eq(event::num_events(), 1);
...
public(package) fun test_update_blocklister<T>(
    ...
    let expected_event = roles::create_blocklister_changed_event<T>(
        old_blocklister,
        new_blocklister,
    );
    assert_eq(event::num_events(), 1);
...
public(package) fun test_update_pauser<T>(
    new_pauser: address,
    roles: &mut Roles<T>,
    scenario: &mut Scenario,
) {
...
public(package) fun test_update_metadata_updater<T>(
    ...
    let expected_event = roles::create_metadata_updater_changed_event<T>(
