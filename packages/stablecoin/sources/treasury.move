...
public(package) fun assert_treasury_cap_exists<T>(treasury: &Treasury<T>) {
    assert!(
        dof::exists_with_type<_, TreasuryCap<T>>(&treasury.id, TreasuryCapKey {}),
        ETreasuryCapNotFound,
    );
}
...
/// - Only callable by the master minter.
/// - Only callable if the Treasury object is compatible with this package.
fun create_mint_cap<T>(treasury: &Treasury<T>, ctx: &mut TxContext): MintCap<T> {
    treasury.assert_is_compatible();
    assert!(treasury.roles.master_minter() == ctx.sender(), ENotMasterMinter);
...
/// - Only callable by the master minter.
/// - Only callable if the Treasury object is compatible with this package.
entry fun remove_controller<T>(treasury: &mut Treasury<T>, controller: address, ctx: &TxContext) {
    treasury.assert_is_compatible();
    assert!(treasury.roles.master_minter() == ctx.sender(), ENotMasterMinter);
...
/// - Only callable by the MintCap's controller.
/// - Only callable if the Treasury object is compatible with this package.
entry fun remove_minter<T>(treasury: &mut Treasury<T>, ctx: &TxContext) {
    treasury.assert_is_compatible();
...
/// - Only callable if the Treasury object is compatible with this package.
entry fun pause<T>(treasury: &mut Treasury<T>, deny_list: &mut DenyList, ctx: &mut TxContext) {
    treasury.assert_is_compatible();
...
/// - Only callable by the pauser.
/// - Only callable if the Treasury object is compatible with this package.
entry fun unpause<T>(treasury: &mut Treasury<T>, deny_list: &mut DenyList, ctx: &mut TxContext) {
    treasury.assert_is_compatible();
    assert!(treasury.roles().pauser() == ctx.sender(), ENotPauser);
    let deny_cap = treasury.borrow_deny_cap_mut();
...
#[test_only]
public(package) fun get_mint_allowances_for_testing<T>(
    treasury: &Treasury<T>,
): &Table<ID, MintAllowance<T>> {
...
#[test_only]
public(package) fun set_compatible_versions_for_testing<T>(
    treasury: &mut Treasury<T>,
    compatible_versions: VecSet<u64>,
) {
...
#[test_only]
public(package) fun create_controller_configured_event<T>(
    controller: address,
    mint_cap: ID,
...
#[test_only]
public(package) fun create_minter_configured_event<T>(
    controller: address,
    mint_cap: ID,
    allowance: u64,
): MinterConfigured<T> {
...
#[test_only]
public(package) fun create_minter_removed_event<T>(
    controller: address,
    mint_cap: ID,
): MinterRemoved<T> {
...
#[test_only]
public(package) fun create_migration_started_event<T>(
    compatible_versions: vector<u64>,
): MigrationStarted<T> {
...
#[test_only]
public(package) fun create_migration_aborted_event<T>(
    compatible_versions: vector<u64>,
): MigrationAborted<T> {
...
#[test_only]
public(package) fun create_migration_completed_event<T>(
    compatible_versions: vector<u64>,
