...
public(package) fun update_master_minter<T>(
    roles: &mut Roles<T>,
    new_master_minter: address,
    ctx: &TxContext,
) {
...
/// - Only callable by the owner.
public(package) fun update_blocklister<T>(
    roles: &mut Roles<T>,
    new_blocklister: address,
    ctx: &TxContext,
) {
...
/// - Only callable by the owner.
public(package) fun update_metadata_updater<T>(
    roles: &mut Roles<T>,
    new_metadata_updater: address,
    ctx: &TxContext,
) {
...
fun update_address<T, K: copy + drop + store>(
    roles: &mut Roles<T>,
    key: K,
    new_address: address,
): address {
...
#[test_only]
public(package) fun create_master_minter_changed_event<T>(
    old_master_minter: address,
    new_master_minter: address,
...
#[test_only]
public(package) fun create_blocklister_changed_event<T>(
    old_blocklister: address,
    new_blocklister: address,
...
#[test_only]
public(package) fun create_pauser_changed_event<T>(
    old_pauser: address,
    new_pauser: address,
): PauserChanged<T> {
...
#[test_only]
public(package) fun create_metadata_updater_changed_event<T>(
    old_metadata_updater: address,
    new_metadata_updater: address,
