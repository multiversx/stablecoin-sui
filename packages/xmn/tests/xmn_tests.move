...
module xmn::xmn_tests {
    ...
    fun init__should_create_correct_number_of_objects() {
        ...
        xmn::init_for_testing(scenario.ctx());
    ...
    fun init__should_create_correct_coin_metadata() {
        ...
        let metadata = scenario.take_shared<CoinMetadata<XMN>>();
        assert_eq(metadata.get_decimals(), 6);
        assert_eq(metadata.get_name(), string::utf8(b"XMN"));
        assert_eq(metadata.get_symbol(), ascii::string(b"XMN"));
        assert_eq(metadata.get_description(), string::utf8(b"<placeholder>"));
        assert_eq(metadata.get_icon_url(), option::some(url::new_unsafe(ascii::string(b"<placeholder>"))));
    ...
    fun init__should_create_regulated_coin_metadata() {
        ...
        scenario.next_tx(DEPLOYER);
        assert_eq(test_scenario::has_most_recent_immutable<RegulatedCoinMetadata<XMN>>(), true);
    ...
    fun init__should_create_shared_treasury_and_wrap_treasury_cap() {
        ...
        scenario.next_tx(DEPLOYER);
        let treasury = scenario.take_shared<Treasury<XMN>>();
    ...
    fun init__should_create_shared_treasury_and_wrap_deny_cap() {
        ...
        let mut treasury = scenario.take_shared<Treasury<XMN>>();
        let mut deny_list = scenario.take_shared<DenyList>();

        treasury.blocklist(&mut deny_list, RANDOM_ADDRESS, scenario.ctx());
        assert_eq(coin::deny_list_v2_contains_next_epoch<XMN>(&deny_list, RANDOM_ADDRESS), true);
    ...
    fun init__should_create_shared_upgrade_service() {   
        ...
        scenario.next_tx(DEPLOYER);
        let upgrade_service = scenario.take_shared<UpgradeService<XMN>>();
