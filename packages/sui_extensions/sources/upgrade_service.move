...
use sui::event;
use sui::package::{UpgradeTicket, UpgradeReceipt, UpgradeCap};
...
entry fun deposit<T>(upgrade_service: &mut UpgradeService<T>, upgrade_cap: UpgradeCap) {
    ...
    let package_address_of_type = address::from_ascii_bytes(type_name.get_address().as_bytes());
...
#[test_only]
public(package) fun add_upgrade_cap_for_testing<T>(
    upgrade_service: &mut UpgradeService<T>,
...
#[test_only]
public(package) fun borrow_upgrade_cap_for_testing<T>(
...
#[test_only]
public(package) fun borrow_upgrade_cap_mut_for_testing<T>(
    upgrade_service: &mut UpgradeService<T>,
): &mut UpgradeCap {
...
#[test_only]
public(package) fun create_upgrade_cap_deposited_event<T>(
    upgrade_cap_id: ID,
...
#[test_only]
public(package) fun create_upgrade_cap_extracted_event<T>(
    upgrade_cap_id: ID,
...
#[test_only]
public(package) fun create_authorize_upgrade_event<T>(
    package_id: ID,
    policy: u8,
): AuthorizeUpgrade<T> {
