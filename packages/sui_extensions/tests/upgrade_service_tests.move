...
use sui::event;
use sui::package::{Self, UpgradeTicket, UpgradeReceipt, UpgradeCap};
use sui::test_scenario::{Self, Scenario};
use sui::test_utils::{assert_eq, destroy, create_one_time_witness};
use sui_extensions::test_utils::last_event_by_type;
use sui_extensions::two_step_role;
use sui_extensions::upgrade_service::{Self, UpgradeService, AdminRole};
...
fun new__should_succeed_and_pass_all_assertions() {
    ...
    destroy(
        test_new(
            &mut scenario,
            create_one_time_witness<UPGRADE_SERVICE_TESTS>(),
            UPGRADE_SERVICE_ADMIN,
        ),
    );
...
fun authorize_upgrade__should_fail_if_sender_is_not_admin() {
    ...
    scenario.next_tx(RANDOM_ADDRESS);
    destroy(
        test_authorize_upgrade<UPGRADE_SERVICE_TESTS>(
            &mut scenario,
...
fun commit_upgrade__should_fail_if_upgrade_cap_and_receipt_are_mismatched() {
    ...
    let mut upgrade_cap_for_random_package = create_upgrade_cap(
        &mut scenario,
        RANDOM_ADDRESS.to_id(),
    );
    let upgrade_ticket_for_random_package = upgrade_cap_for_random_package.authorize(
        package::compatible_policy(),
        TEST_DIGEST,
    );
    let upgrade_receipt_for_random_package = package::test_upgrade(
        upgrade_ticket_for_random_package,
    );
...
fun setup_with_shared_upgrade_service(): Scenario {
    ...
    let mut upgrade_service = test_new(
        &mut scenario,
        create_one_time_witness<UPGRADE_SERVICE_TESTS>(),
        UPGRADE_SERVICE_ADMIN,
    );
...
fun test_deposit<T>(scenario: &Scenario, upgrade_cap: UpgradeCap) {
    ...
    assert_eq(
        object::id(upgrade_service.borrow_upgrade_cap_for_testing()),
        expected_upgrade_cap_id,
    );
...
fun create_upgrade_cap(scenario: &mut Scenario, package_id: ID): UpgradeCap {
...
fun check_upgrade_service_and_upgrade_cap<T>(
    upgrade_service: &UpgradeService<T>,
    package: ID,
    version: u64,
    policy: u8,
) {
